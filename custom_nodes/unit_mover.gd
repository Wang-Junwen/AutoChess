...
func _get_play_area_for_position(global: Vector2) -> int:
	...
	# 遍历 play_areas 数组，通过is_tile_in_bounds方法找到包含 global 的 PlayArea的索引
...
func _reset_unit_to_starting_position(starting_position: Vector2, unit: Unit) -> void:
	...
	unit.reset_after_draging(starting_position)
...
func _on_unit_dropped(starting_position: Vector2, unit: Unit) -> void:
	...
	var invalid_drop := dropped_area_index == -1
	var bench_to_game := old_area_index == 1 and dropped_area_index == 0
	...
	if new_area.unit_grid.is_tile_occupied(new_tile):
		var old_unit: Unit = new_area.unit_grid.get_unit_at(new_tile)
