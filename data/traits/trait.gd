...
func get_unique_unit_count(units: Array[Unit]) -> int:
	units = units.filter(func(unit: Unit) -> bool: return unit.stats.traits.has(self))
...
func get_levels_bbcode(unit_count: int) -> String:
	...
	var reached_level := unit_requirements.filter(func(req: int) -> bool: return req <= unit_count)
	# 高亮已达到的等级
	for i: int in levels:
...
static func get_trait_names(traits: Array[Trait]) -> PackedStringArray:
	...
	for cur_trait: Trait in traits:
		trait_names.append(cur_trait.name)
	return trait_names
