...
const TRAIT_UI = preload("res://scene/trait_ui/trait_ui.tscn")

@export var arena_grid: UnitGrid

@onready var trait_container: VBoxContainer = %TraitsContainer
...
func _delete_orphan_traits() -> void:
	for orphan_traits: Trait in traits_to_update:
		var trait_ui := current_traits[orphan_traits] as TraitUI
		current_traits.erase(orphan_traits)
		trait_ui.queue_free()
