...
func update(units: Array[Unit]) -> void:
	var unique_unit_count = trait_data.get_unique_unit_count(units)
	active_units_label.text = str(unique_unit_count)

	trait_level_labels.text = trait_data.get_levels_bbcode(unique_unit_count)
	active = trait_data.is_active(unique_unit_count)
